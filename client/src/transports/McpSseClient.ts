...
export class McpSseClient extends McpClient {
  ...
  async connectSse(): Promise<void> {
    const baseUrl = this.sseOptions.baseUrl instanceof URL 
    ...
    const transport = new SSEClientTransport(
      baseUrl,
      {
        headers: this.sseOptions.headers,
        fetch: this.sseOptions.fetch,
        EventSource: this.sseOptions.EventSource,
        timeout: this.sseOptions.timeout
      }
    );
